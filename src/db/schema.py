...
class User(Base):
    __tablename__ = "user"
    __table_args__ = {"schema": settings.active_schema}
...
class ChatMessage(Base):
    ...
    session_id = Column(
        UUID(as_uuid=True),
        ForeignKey(f"{settings.active_schema}.chat_session.session_id"),
    )
    ...
    __table_args__ = (
        Index("idx_session_created_at", text("session_id, created_at ASC")),
        {"schema": settings.active_schema},
    )
